...
function Card(e){this.init(e)}var ONE_NUMBER_KEY_CODE=49,TWO_NUMBER_KEY_CODE=50,THREE_NUMBER_KEY_CODE=51,FOUR_NUMBER_KEY_CODE=52,FIVE_NUMBER_KEY_CODE=53,SPACE_KEY_CODE=32;Card.prototype.init=function(e){this.$el=e,this.cacheElements(),this.bindEvents()},Card.prototype.destroy=function(){this.unbindEvents(),this.$el.remove()},Card.prototype.cacheElements=function(){this.$backOverlay=this.$el.find(".card__back-overlay"),this.$backAudio=this.$el.find(".card__back-audio"),this.$playBackAudioBtn=this.$el.find(".card__back-play-audio-btn"),this.$confidence=this.$el.find(".card__confidence"),this.$confidenceButtons=this.$el.find(".card__confidence-button"),this.$revealAnswerBtn=this.$el.find(".card__reveal-answer-btn")},Card.prototype.bindEvents=function(){this.$revealAnswerBtn.on("click",{self:this},this.revealAnswerBtnClickEventHandler),this.$playBackAudioBtn.on("click",{self:this},this.playBackAudioBtnClickEventHandler),$(document).on("keydown",{self:this},this.documentKeyDownEventHandler),$(document).on("keyup",{self:this},this.documentKeyUpEventHandler)},Card.prototype.unbindEvents=function(){},Card.prototype.revealAnswerBtnClickEventHandler=function(e){var t=e.data.self;t.openAnswer()},Card.prototype.isAnswerOpened=function(){return this.$backOverlay.is(".m-active")},Card.prototype.openAnswer=function(){var e=this;this.$backOverlay.removeClass("m-active"),this.playBackAudio(),setTimeout(function(){e.openRatings()},0)},Card.prototype.playBackAudioBtnClickEventHandler=function(e){var t=e.data.self;t.playBackAudio()},Card.prototype.documentKeyDownEventHandler=function(e){var t=e.data.self;if(t.isAnswerOpened()&&e.which===SPACE_KEY_CODE?t.openAnswer():e.which===SPACE_KEY_CODE&&t.playBackAudio(),t.$confidence.is(".m-active"))switch(e.which){case ONE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=1]").addClass("m-active");break;case TWO_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=2]").addClass("m-active");break;case THREE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=3]").addClass("m-active");break;case FOUR_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=4]").addClass("m-active");break;case FIVE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=5]").addClass("m-active")}},Card.prototype.documentKeyUpEventHandler=function(e){var t=e.data.self;if(t.$confidence.is(".m-active"))switch(t.$confidenceButtons.removeClass("m-active"),e.which){case ONE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=1]").trigger("click");break;case TWO_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=2]").trigger("click");break;case THREE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=3]").trigger("click");break;case FOUR_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=4]").trigger("click");break;case FIVE_NUMBER_KEY_CODE:t.$confidenceButtons.filter("[value=5]").trigger("click")}},Card.prototype.playBackAudio=function(){this.$backAudio.length&&this.$backAudio[0].play()},Card.prototype.openRatings=function(){this.$confidence.addClass("m-active"),this.$confidence.fadeIn(250)};
$(document).ready(function(){var n=$(".card");n.length>0&&n.each(function(){new Card($(this))})});